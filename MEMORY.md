...
- First-choice occupancy/onboarding flow = house prefilled form or personal registration key generated from MC. Prefill only known fields (house address, room code, price, start date). Never expose the house code to occupants. If using a personal key, generate it in MC and assign the occupant's number so the link stays private.
