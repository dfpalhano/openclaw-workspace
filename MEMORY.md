...
- First-choice occupancy/onboarding flow = personal registration key generated from MC, with prefilled known fields (house address, room code, price, start date). Never expose the house code to occupants. Use the house prefilled form only when the personal-key flow is unavailable.
